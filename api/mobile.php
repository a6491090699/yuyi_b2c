...
if(isset($uri) && is_url($uri)) {
	if($action == 'device') {
		...
		if(DT_DOMAIN) {
			...
			if(substr($uri, 0, strlen(DT_MOB)) == DT_MOB) $url = $uri;
		}
	} else {
		if(substr($uri, 0, strlen(DT_MOB)) == DT_MOB) $url = $uri;
	}
...
if($action == 'device') {
	if(isset($uri) && is_url($uri) && substr($uri, 0, strlen(DT_PATH)) == DT_PATH) $url = $uri;
} else {
	if(isset($uri) && is_url($uri) && substr($uri, 0, strlen(DT_MOB)) == DT_MOB) $url = $uri;
}
if($action == 'pc') {
